...
def parse_3des_key(key_input):
    """
    Parses key into 24 bytes (192 bits) for 3-key Triple DES.
    Handles 48-char hex strings or 24-char raw text.
    """
    if len(key_input) == 48:
        try:
    ...
    if len(key_bytes) not in (16, 24):
        raise ValueError("3DES key must be 16 bytes (Option 2) or 24 bytes (Option 1) / 48 hex chars.")

    return DES3.adjust_key_parity(key_bytes)


def triple_des_encrypt(plaintext_str, key_str):
    """Encrypts plaintext using Triple DES (ECB mode with PKCS7 padding)."""
...
def triple_des_decrypt(ciphertext_bytes, key_str):
    """Decrypts Triple DES ciphertext and removes PKCS7 padding."""
    key = parse_3des_key(key_str)
